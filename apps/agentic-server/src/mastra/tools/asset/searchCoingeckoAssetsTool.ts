  ...
  try {
    ...
    for (const [platformId, platform] of Object.entries(coin.detail_platforms)) {
      if (platform?.contract_address && platform.contract_address !== NULL_ADDRESS) {
        ...
        const unifiedNetwork = Object.entries(UNIFIED_TO_SEARCH_PLATFORM).find(([, id]) => id === platformId)?.[0] as
          | UnifiedNetwork
          | undefined
...
export const searchCoingeckoAssetsOutput = z.object({
  ...
  alternativeNetworks: z
    .record(z.string(), z.array(z.string()))
    .optional()
    .describe('Map of asset names to networks where they were found but not returned due to network filtering'),
})
