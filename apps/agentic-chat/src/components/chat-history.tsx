...
export const ChatHistory = () => {
  ...
  const getThreadTitle = useCallback(
    (threadId: string) => {
      ...
      const firstHumanMessage = thread.messages.find(msg => msg.role === 'user')
      if (!firstHumanMessage?.content) return 'New Thread'
      return `${firstHumanMessage.content.slice(0, 30)}${firstHumanMessage.content.length > 30 ? '…' : ''}`
    },
    [threads]
  )
            ...
            {sortedThreadIds.map(threadId => (
              <SidebarMenuItem
                ...
                className={`cursor-pointer text-sm px-4 py-2 truncate ${
                  activeThreadId === threadId ? 'bg-sidebar-accent text-sidebar-accent-foreground font-semibold' : ''
                }`}
