...
const ToolMessage: React.FC<{ message: any; toolCall: OpenAIToolCall | undefined }> = ({ message, toolCall }) => {
    ...
    <div className="flex flex-col items-start max-w-[75%]">
      ...
      <div className="w-full mb-2 border rounded-lg overflow-hidden bg-muted">
        ...
        {Object.entries(args).map(([key, value]) => (
          <div key={key} className="flex justify-between items-center px-3 py-2 border-b last:border-b-0">
            <span className="font-semibold text-xs text-muted-foreground">{key}</span>
            <span className="text-xs text-muted-foreground/80 break-all">{typeof value === 'string' ? value : JSON.stringify(value)}</span>
          </div>
        ))}
      </div>
      {/* Tool Result Table */}
...
const ChatMessage: React.FC<{ message: any, toolCall: OpenAIToolCall | undefined }> = ({ message, toolCall }) => {
  ...
  return (
    <ScrollArea className="flex-1 p-4">
      <div className="space-y-4">
        ...
        const maybeToolCall = toolCalls.find((call) => call.id === message.tool_call_id);
        return (
          <ChatMessage key={message.id} message={message} toolCall={maybeToolCall} />
        )})}
      </div>
    </ScrollArea>
  );
