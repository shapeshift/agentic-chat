...
function cancelStateToPersistedState(
  ...
  cancelOutput: CancelStopLossOutput | null,
...
function useCancelStopLossExecution(
  toolCallId: string,
  toolState: DynamicToolUIPart['state'],
  cancelData: CancelStopLossOutput | null
) {
  ...
  const { state } = useToolExecutionEffect(toolCallId, cancelData, initialCancelState, async (data, setState) => {
    ...
    try {
      ...
      toast.success('Stop-loss order cancelled successfully')
    } catch (error) {
...
export function CancelStopLossUI({ toolPart }: ToolUIComponentProps<'cancelStopLossTool'>) {
  ...
  const { error, steps, cancelTxHash } = useCancelStopLossExecution(toolCallId, state, cancelData)
    ...
    <TxStepCard.Root>
      <TxStepCard.Header>
        <TxStepCard.HeaderRow>
          <div className="text-xs text-muted-foreground font-normal">Cancel Stop-Loss</div>
