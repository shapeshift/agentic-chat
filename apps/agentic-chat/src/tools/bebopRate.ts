...
import type { Asset, BebopQuote, BebopResponse, PortalsToken } from '@shapeshiftoss/types'
...
export const getBebopRate = async ({
  ...
  >
  const { sellAmountCryptoPrecision, fromAsset, toAsset, fromAddress, chain } = typedToolCall.args
  ...
  const sellAmountCryptoBaseUnit = toBaseUnit(sellAmountCryptoPrecision, fromAsset.decimals)

  // Convert ETH symbol to Bebop's ETH marker address
  const sellTokenAddress = getAddress(
    fromAsset.symbol.trim().toUpperCase() === 'ETH' ? BEBOP_ETH_MARKER : fromAsset.address
  )
  const buyTokenAddress = getAddress(toAsset.symbol.trim().toUpperCase() === 'ETH' ? BEBOP_ETH_MARKER : toAsset.address)

  const env = import.meta?.env ? import.meta.env : process.env

  const BEBOP_API_KEY = env.VITE_BEBOP_API_KEY || env.BEBOP_API_KEY

  const url = `https://api.bebop.xyz/router/${bebopChainsMap[chain] ?? chain}/v1/quote`
