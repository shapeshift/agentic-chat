...
import type { ASSET_NAMESPACE, ASSET_REFERENCE } from './constants.js';
...
import type { Nominal } from './utils.js';

export type AssetId = Nominal<string, 'AssetId'>;

export type AssetNamespace =
  (typeof ASSET_NAMESPACE)[keyof typeof ASSET_NAMESPACE];

export type AssetReference =
  (typeof ASSET_REFERENCE)[keyof typeof ASSET_REFERENCE];
...
const isToAssetIdWithChainIdArgs = (
  args: ToAssetIdArgs
): args is ToAssetIdWithChainId => !!args.chainId;
  ...
  const assetReferenceNormalized = (() => {
    const assertContractAddress = (address: string) => {
      if (!address.startsWith('0x'))
        throw new Error(
          `toAssetId: assetReference must start with 0x: ${assetReference}`
        );
    ...
    switch (assetNamespace) {
      ...
      case 'bep721':
      case 'bep1155': {
        // caip-29 (https://github.com/ChainAgnostic/CAIPs/blob/master/CAIPs/caip-29.md) // caip-22 (https://github.com/ChainAgnostic/CAIPs/blob/master/CAIPs/caip-22.md)
