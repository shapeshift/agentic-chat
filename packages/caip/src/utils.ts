...
import { fromAccountId } from './accountId'
import type { AssetId } from './assetId'
import { fromAssetId, toAssetId } from './assetId'
...
export const accountIdToChainId = (accountId: AccountId): ChainId => fromAccountId(accountId).chainId

export const accountIdToSpecifier = (accountId: AccountId): string => fromAccountId(accountId).account

export const isValidChainPartsPair = (chainNamespace: ChainNamespace, chainReference: ChainReference) =>
  constants.VALID_CHAIN_IDS[chainNamespace]?.includes(chainReference) || false

export const generateAssetIdFromCosmosSdkDenom = (denom: string, nativeAssetId: AssetId): AssetId => {
