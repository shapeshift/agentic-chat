...
const isToAssetIdWithChainIdArgs = (args: ToAssetIdArgs): args is ToAssetIdWithChainId => !!args.chainId
...
export const toAssetId: ToAssetId = (args: ToAssetIdArgs): AssetId => {
  ...
  if (!VALID_ASSET_NAMESPACE[chainNamespace].includes(assetNamespace) || !isAssetNamespace(assetNamespace))
  ...
  const assetReferenceNormalized = (() => {
    const assertContractAddress = (address: string) => {
      if (!address.startsWith('0x')) throw new Error(`toAssetId: assetReference must start with 0x: ${assetReference}`)
