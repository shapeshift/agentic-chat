...
export const isChainNamespace = (
  ...
  Object.values(CHAIN_NAMESPACE).includes(
    maybeChainNamespace as ChainNamespace
  );

export const isChainReference = (
  maybeChainReference: ChainReference | string
): maybeChainReference is ChainReference =>
  Object.values(CHAIN_REFERENCE).includes(
...
export const isAssetNamespace = (
  ...
  Object.values(ASSET_NAMESPACE).includes(
    maybeAssetNamespace as AssetNamespace
  );

export const isAssetReference = (
  maybeAssetReference: AssetReference | string
): maybeAssetReference is AssetReference =>
  Object.values(ASSET_REFERENCE).includes(
    maybeAssetReference as AssetReference
  );

// NOTE: perf critical - benchmark any changes
export const isAssetId = (
  maybeAssetId: AssetId | string
): maybeAssetId is AssetId => {
...
export const isChainId = (
  ...
  const { chainNamespace, chainReference } = fromChainId(
    maybeChainId as ChainId
  );
  return !!VALID_CHAIN_IDS[chainNamespace]?.includes(chainReference);
};

export const isChainIdParts = (
